---
title: "Hermiston Ordination"
author: "Sabrina Heitmann"
date: "12/6/2021"
output: html_document
---

****Added percent N and percent C to the ordination but didn't change it. Haven't completed for Corvallis yet.

****Might need to unhash - 11/25/22
```{r}
library(vegan)
library(tidyverse)
library(phyloseq)
...
```{r}
phy.corv <- subset_samples(phy, Garden == "Hermiston")
...
which(!phyloseq_genotype$Genotype2 %in% ltd_genotype$Genotype) 
#index: 12 = BESC-1212
#index: 36 = BESC-273
#index: 20 = BESC-1229
#index: 51 = BESC-86

#add genotypes back in
ltd_avg <- rbind(c("BESC-1212", NA, NA, NA, NA, NA, NA, NA, NA, "FALSE", "FALSE"), ltd_avg)
ltd_avg <- rbind(c("BESC-273", NA, NA, NA, NA, NA, NA, NA, NA, "FALSE", "FALSE"), ltd_avg)
ltd_avg <- rbind(c("BESC-1229", NA, NA, NA, NA, NA, NA, NA, NA, "FALSE", "FALSE"), ltd_avg)
ltd_avg <- rbind(c("BESC-86", NA, NA, NA, NA, NA, NA, NA, NA, "FALSE", "FALSE"), ltd_avg)
...
Removed overlay for Posy (but reverted)
```{r}
set.seed(108)

phy.dist <- merged_genotype %>% phyloseq::distance("bray")
phy.ord <- metaMDS(phy.dist, k=2, trymax = 50, engine = "monoMDS")
...
#P-values revert back so need to adjust again
ef.adj <- ef 
pvals.adj <- p.adjust (ef$vectors$pvals, method = 'fdr')
ef.adj$vectors$pvals <- pvals.adj
ef.adj
...
data.scores$Garden = "Hermiston"
data.scores
#write.csv(data.scores, file = "output/csvs/herm_nmds.csv") Check back
...
#changed vector plot fill to .2 because they were distorting samples in ordination
#removed ordiArrowMul
#ef_coord_cont = as.data.frame(vegan::scores(ef.adj, "vectors")) * ordiArrowMul(ef.adj, fill = .3)
ef_coord_cont= as.data.frame(vegan::scores(ef.adj, "vectors")) #* ordiArrowMul(ef.adj)
#ef_coord_cat = as.data.frame(scores(ef.adj, "factors")) * ordiArrowMul(ef)

#?ordiArrowMul
#ef_coord_cat %<>%
#  rownames_to_column("name") %>%
#  filter(!grepl("Region", name)) %>%
#  mutate(name = str_remove(name, "Population")) %>%
...
p <-ggsave("output/figs/figure5b.png")
